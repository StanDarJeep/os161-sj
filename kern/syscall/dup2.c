...
int sys__dup2(int oldfd, int newfd, int *retval) {
    ...
    // Check if either fd is invalid
    if (oldfd < 0 || newfd < 0 || newfd >= OPEN_MAX || oldfd >= OPEN_MAX) {
        return EBADF;
    }

    // Handle the trivial case where the fd's are equal
    if (oldfd == newfd) {
        *retval = newfd;
        return 0;
    }

    // Check that the old fd is still in use
    ...
    // Duplicate the old fd into newfd in the fd_table
